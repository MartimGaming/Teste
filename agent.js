...
const SUPABASE_URL = process.env.NOVAPANEL_SUPABASE_URL || "https://aramxmrclrspmkhvpket.supabase.co";
const AGENT_TOKEN = process.env.NOVAPANEL_AGENT_TOKEN || "dc99db843aa87b3afa293344bc97b51ed80400812f4316eb2aef55a1f0702634";
...
async function reportMetrics() {
  ...
  lastReportedAddr = publicAddress;
  lastReportedAuth = authUrl;
}
// Force-flush métricas quando algo importante muda (IP do playit, auth url)
function maybeFlushOnChange() {
  if (publicAddress !== lastReportedAddr || authUrl !== lastReportedAuth) {
    reportMetrics().catch(() => {});
  }
}
...
async function flushLogs() {
  ...
  const lines = LOG_BUF.splice(0, Math.min(LOG_BUF.length, 50))
    .map(cleanLine).filter((l) => l && l.length > 0);
...
async function startServer() {
  ...
  mc.stderr && mc.stderr.on("data", (d) => { const s = cleanLine(d.toString()); if (s) LOG_BUF.push("[STDERR] " + s); });
  mc.on("exit", (code) => {
...
// Heartbeat 2s para realtime instantâneo
setInterval(reportMetrics, 2000);
